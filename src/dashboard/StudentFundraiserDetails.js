...
export default function StudentFundraiserDetails() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.REACT_APP_API_URL}/fundraiser/${fundraiserId}`)
      .then((res) => {
    ...
    >
      ...
      >
        <div className="d-flex flex-row justify-content-start">
          ...
          <Link
            className="ms-4 mt-2"
            to={`/dashboard/editfund/${fundraiser?.id}`}
          >
            Edit
          </Link>
        </div>
        ...
        <ProgressBar
          now={
            (fundraiser?.amount_donated / fundraiser?.scholarship_amount) * 100
          }
          color="#4992e9"
        />
        ...
        <FaRegCalendarAlt /> &nbsp; {moment(fundraiser?.end_date).format(
          "LL"
        )}{" "}
        <br />
      </div>
